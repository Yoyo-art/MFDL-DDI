...
def get_ecfp4_fingerprint(smiles, radius=2, n_bits=2048):
    mol = Chem.MolFromSmiles(smiles)
    fp = AllChem.GetMorganFingerprintAsBitVect(mol, radius, nBits=n_bits)
    np_fp = np.zeros((1,))
    AllChem.DataStructs.ConvertToNumpyArray(fp, np_fp)
    return np_fp


def calculate_jaccard_distance(fp1, fp2):
...
def generate_pair_triplets(args):
    ...
    data = pd.read_csv(args.dataset_filename, delimiter=args.delimiter)
    for id1, id2, relation, smiles1, smiles2 in zip(data[args.c_id1], data[args.c_id2], data[args.c_y], data[args.c_s1], data[args.c_s2]):
        relation -= 1
        drug_smile_dict[id1] = smiles1
        drug_smile_dict[id2] = smiles2
    ...
    fingerprints = {}
    for id in drug_ids:
        fingerprints[id] = get_ecfp4_fingerprint(drug_smile_dict[id])
    n = len(remaining_drug_ids)
    distance_matrix = {}
    for id1 in remaining_drug_ids:
        count = 0
        for id2 in drug_ids:
            distance = calculate_jaccard_distance(fingerprints[id1], fingerprints[id2])
            count += distance
        distance_matrix[id1] = count

    remaining_drug_ids = sorted(distance_matrix)
    ...
    for pos_tups, desc in [
        ...
        filename = f'{args.dirname}/inductive/structure_based/pair_pos_neg_triples-fold{args.seed}-{desc}.csv'
